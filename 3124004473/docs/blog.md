...
> **本次作业 GitHub 项目链接：** <!-- 粘贴你的仓库地址，例如 https://github.com/你的用户名/仓库名 -->
>
...
| 测试文件 | 用例数 | 测什么 |
| --- | ---: | --- |
| `test_exceptions.py` | 9 | 异常体系的契约（退出码、继承、消息） |
| `test_normalize.py` | 25 | 归一化与等价性回归 |
| `test_tokenize.py` | 12 | n-gram 切分与计数的边界 |
| `test_similarity.py` | 62 | 四种度量的定义、边界与正确性对拍 |
| `test_textio.py` | 40 | 文件读写的各种故障模式 |
| `test_engine.py` | 44 | 引擎的性质（端点、单调性、不变性） |
| `test_cli.py` | 20 | 命令行黑盒端到端 |
| **合计** | **212** | 全部通过 |
...
最终交付：212 个单元测试、100% 行覆盖率、pylint 10.00/10、
